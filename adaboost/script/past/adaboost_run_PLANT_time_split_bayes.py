...
def make_inner_cv(df: pd.DataFrame, n_splits: int, seed: int) -> StratifiedGroupKFold:
    if n_splits < 2:
...
def run_one_split(
    ...
    *,
    ...
    cv = make_inner_cv(tune_df, args.cv, args.random_seed)
    bayes_search = make_bayes_search(args.random_seed, args.n_iter, cv, args.n_jobs, args.verbose)

    print("Beginning BayesSearchCV...")
    fit_kwargs = {
        "groups": tune_df["virus"].to_numpy(),
    }
